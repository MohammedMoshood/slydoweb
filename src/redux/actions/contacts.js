...
export const getContacts = (page) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/v1/user/contacts?page=${page}&page_size=10`, {
      headers: {
...
// blocked contact
export const getBlockedContacts = (page) => async (dispatch) => {
  dispatch({type: CONTACT_LOADING})
  try {
    const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/v1/user/contacts/list-block-contact/?page=${page}&page_size=10`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + JSON.parse(localStorage.getItem("accessToken")).refresh,
      },
    ...
    if (response.data?.results.length) {
      const blockedcontacts = response.data.results.map((ob) => ({
        name: ob.full_name,
        username: ob.username,
      }));
      const blockedcontactsData = {
        blockcontact: blockedcontacts,
        count: response.data.count,
      }
      dispatch({
        type: BLOCK_CONTACT,
        payload: blockedcontactsData,
      });
    } else {
      dispatch({
        type: BLOCK_CONTACT,
