...
import { getPaymentRequest, getTransactions, getPayouts, getInvoice, getBankAccounts } from "../../redux/actions/payments";
              ...
              loading ? (
                ...
                payouts?.payouts?.map((item, index) => {
                  ...
                  return (
                    <tr key={index}>
                      <td>
                        {item?.customer_bank_account?.bank_account}
                      </td>
                      <td>
                        {item?.customer_bank_account?.account_number}
                      </td>
                      <td style={{width: '20%'}}>
                        {
                          (item?.amount/100).toLocaleString("en-NG", {
                      ...
                      <td>
                        <Badge 
                          pill 
                          className={`tb-status btn-dim ml-1 btn-${
                            item?.status === "Paid" ? "success" :  item?.status === "Pending" ? "danger" : item?.status === "Processing" ? "warning" : item.status === "Awaiting Payment" ? "primary" : item.status === "Canceled" ? "danger" : "Complete" ? "success" : "Pending" ? "danger" : "danger"
                          }`}
                        >
                          {item?.status}
                        </Badge>
                      ...
                      <tr key={item?.id}>
                        ...
                        <td>
                          <Badge 
                            pill 
                            className={`tb-status btn-dim ml-1 btn-${
                              item?.status === "Paid" ? "primary" :  item?.status === "Unpaid" ? "danger" : item?.status === "Processing" ? "warning" : item.status === "Awaiting Payment" ? "primary" : item.status === "Canceled" ? "danger" : "Complete" ? "success" : "Pending" ? "danger" : "danger"
                            }`}
                          >
                            {item?.status}
                          </Badge>
                        </td>
                        <td>{dueDateFormat} <span className="text-secondary" style={{fontWeight: 500}}>• {dueDateFormat}</span></td>
                        <td>{invoiceDateFormat} <span className="text-secondary" style={{fontWeight: 500}}>• {invoiceDateFormat}</span></td>
                        {/* <td>{item?.description}</td> */}
                        {/* <td>{item?.paid === true ? "Paid" : "Not paid"}</td> */}
                        <td>
