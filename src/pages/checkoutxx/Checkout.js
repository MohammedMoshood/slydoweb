...
import { filterStatus, checkoutData } from "./CheckoutData";
...
const Checkout = () => {
  const { contextData } = useContext(CheckoutContext);
  ...
  useEffect(() => {
    if (onSearchText !== "") {
      const filteredObject = checkoutData.filter((item) => {
        return (
